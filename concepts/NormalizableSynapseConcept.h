 ...
 *  \li precision_t get_w_max() const
 *  \li precision_t get_weight() const
 *  \li void set_weight(precision_t value)
 */
template <typename T>
concept NormalizableSynapseConcept = requires(T s, const T t) {
    typename T::precision_t;
    { s.set_weight(typename T::precision_t{}) } -> std::same_as<void>;
    { t.get_w_max() } -> std::convertible_to<typename T::precision_t>;
    { t.get_weight() } -> std::convertible_to<typename T::precision_t>;
};
